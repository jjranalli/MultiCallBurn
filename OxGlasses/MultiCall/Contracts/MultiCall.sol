...
contract MultiCall is AccessControl, ReentrancyGuard {
    ...
    constructor(address _oxGlassesContract) {
        require(
            ERC165Checker.supportsInterface(_oxGlassesContract, type(IERC721ABurnable).interfaceId),
            "Contract at address does not implement IERC721ABurnable"
        );
    ...
    function checkBalance(uint256 tokenId) public view returns (uint256) {
        return oxGlassesContract.balanceOf(msg.sender, tokenId);
    }
}
